...
parser.add_argument("--checkpoint", type=str,default="tmp/", help="训练模型的保存位置")# paddle的学习率使用策略和pytorch不一样
parser.add_argument("--logdir", type=str,default="tmp/", help="训练日志存储路径")# paddle的学习率使用策略和pytorch不一样
...
parser.add_argument("--gpu", action="store_true", help="使用gpu训练")# paddle的学习率使用策略和pytorch不一样
...
def train(train_loader:DataLoader, model:nn.Layer,loss_function, optimizer, epoch, args):
    ...
    return {"loss":avgloss,"acc":acc,"epoch":epoch}
...
def eval(test_loader, model:nn.Layer,loss_function, epoch, args):
    ...
    with LogWriter(logdir=args.logdir) as writer:
        # use `add_scalar` to record scalar values
        writer.add_scalar(tag="eval/acc", step=epoch, value=avgloss)
        writer.add_scalar(tag="eval/loss", step=epoch, value=avgacc)
    return {"loss":avgloss,"acc":acc,"epoch":epoch}
...
def main():
    ...
    train_data,test_data=pipline_data_mlp(dataset_dir=args.data,expansion=3)
    ...
    for epoch in range(args.start_epoch, args.epochs):
        train(train_loader, cls_model,loss_function, optimizer, epoch, args)
        lr.step(epoch)# 更新一下学习率
        with paddle.no_grad():
            eval(test_loader,cls_model,loss_function,epoch)
